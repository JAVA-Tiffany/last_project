...
public class MailController {
   ...
   @ResponseBody
   public String mailSending(@RequestParam String email, @RequestParam String ch, @RequestParam(required = false) String id) {
      if (ch.equals("인증")) {
         return mailService.mailSending(email, "인증","");
      } else {
         return mailService.mailSending(email, "비번",id);
      }
   }
